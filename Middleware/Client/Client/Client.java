...
{
	//middleware resource manager -> set to middleware instance in RMIClient
	...
	public void execute(Command cmd, Vector<String> arguments) throws RemoteException, NumberFormatException
	{
		switch (cmd)
		{
			...
			case AddCustomer: {
				...
				int id = toInt(arguments.elementAt(1));
				int customer = m_resourceManager.addCustomer(id);
			...
			case AddCustomerID: {
				...
				if (m_resourceManager.addCustomerID(id, customerID)) {
			...
			case QueryCustomer: {
				...
				String bill = m_resourceManager.queryCustomer(id, customerID);
			...
			case Analytics: {
				checkArgumentsCount(3, arguments.size());
				System.out.println("Analytics for items with quanitiy less than" + arguments.elementAt(2) + " [xid=" + arguments.elementAt(1) + "]");

				int id = toInt(arguments.elementAt(1));
				int threshhold = toInt(arguments.elementAt(2));

				String results = m_resourceManager.Analytics(id, threshhold);
			...
			case Summary: {
				...
				System.out.println("Summary of the customers accounts [xid=" + arguments.elementAt(1) + "]");

				int id = toInt(arguments.elementAt(1));

				String summary = m_resourceManager.Summary(id);
